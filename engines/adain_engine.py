...
class AdaINEngine(BaseStyleEngine):
    ...
    def _load_cached_style_if_exists(self) -> None:
        """Load pre-encoded style (ghibli_style.pt etc)."""
    ...
    def stylize(
        ...
        **kwargs
        ...
        """Apply AdaIN style transfer to content image."""
        ...
        with torch.no_grad():
            ...
            if self._cached_style_stats is not None:
                ...
                if style_image is not None:
                    if isinstance(style_image, bytes):
                        style_image = Image.open(io.BytesIO(style_image)).convert("RGB")
                    style = self._style_transform(style_image).unsqueeze(0).to(self.device)
